...
class HarDNet(nn.Module):
    def __init__(self, depth_wise=False, arch=68, pretrained=True, weight_path='', input_height=512):
        super().__init__()
        first_ch  = [32, 64]
        second_kernel = 3
        max_pool = True
        ...
        if pretrained:
          if hasattr(torch, 'hub'):
            ...
            self.load_state_dict(torch.hub.load_state_dict_from_url(checkpoint, progress=False, map_location="cpu"))
          else:
        ...
        if arch == 68:
            self.out_channels = [128, 320, 640, 1024]
            self.checkpoint = [4, 9, 12, 15]
        elif arch == 85:
            self.out_channels = [192, 320, 720, 1280]
            self.checkpoint = [4, 9, 14, 18]
        else:
